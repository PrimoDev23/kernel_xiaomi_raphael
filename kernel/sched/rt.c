...
extern int update_rt_rq_load_avg(u64 now, int cpu, struct rq *rq, int running);
...
pick_next_task_rt(struct rq *rq, struct task_struct *prev, struct rq_flags *rf)
{
	...
	if (p)
		update_rt_rq_load_avg(rq_clock_task(rq), cpu_of(rq), rq,
	...
	if (rq->curr->sched_class != &rt_sched_class)
		update_rt_rq_load_avg(rq_clock_task(rq), cpu_of(rq), rq, 0);
...
static void put_prev_task_rt(struct rq *rq, struct task_struct *p)
{
	...
	update_rt_rq_load_avg(rq_clock_task(rq), cpu_of(rq), rq, 1);
